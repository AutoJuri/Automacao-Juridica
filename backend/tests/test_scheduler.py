...
from app.core.scheduler import JOB_ID_CICLO_PIPES, JOB_ID_RENOVACAO_DIARIA, TIMEZONE_BRASILIA
...
class TestIniciarScheduler:
    ...
    async def test_e_idempotente_nao_duplica_jobs(self, fresh_scheduler):
        ...
        assert len(fresh_scheduler.get_jobs()) == 2

    @pytest.mark.asyncio
    async def test_ambos_os_jobs_tem_max_instances_1(self, fresh_scheduler):
        scheduler_module.iniciar_scheduler()

        for job_id in (JOB_ID_RENOVACAO_DIARIA, JOB_ID_CICLO_PIPES):
